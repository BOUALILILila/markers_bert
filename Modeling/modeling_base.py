...
from transformers import AdamW, get_linear_schedule_with_warmup, BertConfig, BertForSequenceClassification
...
from modeling_utils_base import LazyTextDataset
...
def evaluate(args, model, tokenizer, prefix="", set_name='dev', global_step):
    ...
    eval_dataset_paths = (f'{args.data_dir}/doc2query_run/base/run_dev.csv',) 
    ...
    for dataset_path, eval_output_dir in zip(eval_dataset_paths,eval_outputs_dirs):
        ...
        if args.msmarco_output:
            msmarco_file = open(
                f"{args.output_dir}/base/checkpoint-{global_step}/msmarco_predictions_{set_name}.tsv", "w")
        query_docids_map = []
        with open(
            args.data_dir + "/doc2query_run/base/query_doc_ids.top1000.doc2query.dev.small_full.txt") as ref_file:
