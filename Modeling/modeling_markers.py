...
from modeling_utils_markers import LazyTextDataset
...
def evaluate(args, model, tokenizer, prefix="", set_name='dev', global_step):
    ...
    eval_dataset_paths = (f'{args.data_dir}/doc2query_run/markers/run_dev.csv',) #eval path / dev path/both
    ...
    for dataset_path, eval_output_dir in zip(eval_dataset_paths,eval_outputs_dirs):
        ...
        if args.msmarco_output:
            msmarco_file = open(
                f"{args.output_dir}/markers/checkpoint-{global_step}/msmarco_predictions_{set_name}.tsv", "w")
        query_docids_map = []
        with open(
